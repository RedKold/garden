...
之前的工作中，我们终于实现了多用户程序的分时运行，当然要展示一下！
这里，我们维护一个前台程序的 `PCB` 进程上下文描述块: `fg_pcb`
...
然后在 `event_read` 中修改这一部分，让 `fg_pcb` 可以灵活切换（通过 `F1, F2...`）

这里可以注意一下，我们在 `proc.c` 定义的 `pcb` 之类的都是 `static` 的，这限制了操作权限（`proc.c ` 编译单元）我们可以用一个函数来修改，正体现了 C 语言朴素的 OOP 思想。
...
内核程序(hello)和用户程序 (pal) 的运行，要求二者互不影响，这呼唤一个 VME 系统，管理不同层级程序的权限。VME (虚拟内存)系统的灵魂。

在调用 `context_uload` 载入程序的时候（如 PAL），Nanos-lite 为用户程序分配空间，首先其调用 `protected` 来保护内核的空间映射被每个用户程序所持有，然后对用户栈进行映射。

做完用户栈映射之后，我们调用 `loader`，并将客户程序的各段分配虚页，通过 `map` 维护映射。

`map` 维护的映射，我们在 NEMU 中通过 `isa_translate` 函数获取真正的物理地址来访问。

在完成了虚拟内存的映射后，我们还需要保证分时能够正常运行，即能正确恢复上下文 Context，在这里我们为 cpu添加了 `satp` 寄存器，为 Context 结构体添加了 `pdir` 来管理虚拟页的基页。在 AM 层，通过 `trap.S` ，我们在发生 `yield` 或者外部中断时候，可以正确的恢复上下文，从而正确维护不同进程要“**阅读**”的正确页表，保证内存互不干扰，映射正确。

我们添加了时钟中断作为唯一的外部中断（*来自外部的声音*），硬件上，我们周期性的发送时钟中断的信号，当 NEMU 接收到这个信号，就会 Raise an Interrupt 来中断，AM 根据中断号 `0x80000007` 分发事件，Nanos-lite 根据事件号来进行 `schedule` 完成调度的切换。

调度的切换是通过 `set_satp` 和 `get_satp` 和 `trap.S` 等汇编语句完成的，汇编语句直接和 NEMU 沟通，达到切换特权层级 `c->np` 和切换页表 `cpu.csrs.satp` 和切换 `PC` 等效果。

**在用户栈的切换方面**，我们想到内核的虚拟地址空间 (kas) 被所有用户进程共有，故用内核栈维护不同用户程序的虚拟地址空间描述符 (`pdir`)。

在需要不同用户进程调度，我们先进入内核空间修改 pdir 和特权级 np pp 等信息，然后再还原栈。

这样，就完成了 `hello` 和 `pal` 的分时运行。
...
```
kasumi@iZuf6d880j0fyz9px7feiaZ:~/dev/ics$ vim segment.c
kasumi@iZuf6d880j0fyz9px7feiaZ:~/dev/ics$ gcc segment.c -o segment_test
kasumi@iZuf6d880j0fyz9px7feiaZ:~/dev/ics$ ./segment_test
...
From the aspect of Program, `"abc"` is a const string, or const **char** type pointer. So it cannot be modifed by other code, such as `p[0] = 'A'`;

From the aspect of interpreter, `"abc"` , as a const string value, it will store in `ELF` 's `.rodata` section, instead of `.data` section.

From the aspect of Linker, `"abc"`, who is in `.data` section, will be placed at `.text` segment. Page System will give `.text` a specific priviledge `R(Read-only)`. When other code try to modify the value at `"abc"` 's address, it will access the `.text` segment and trigger a *Segmentation Fault*

From the aspect of run-time environment and system, our *Nanos-lite* will map that part as read-only when load the *ELF* file. (Though **PA** not implement that part)

From the aspect of hardware, our *NEMU* will check the *Read-Only* valid bit, when read the page table. If the page table's entry (**PTE**) shows that page cannot be modified, then we throw a Page Fault

From the aspect of Interrupt Handling, we back to kernel program and check it's a valid address,  but a invalid write to read-only memory, so we send SIGSEGV signal to stop the program and print "Segmentation Fault".

合适的工具：gdb，汇编语句，查看 elf 头



# 尾声
> 这是一次难忘的旅程，受了很多苦，也成长了很多。可喜可贺可喜可贺。下学期的操作系统继续努力！
