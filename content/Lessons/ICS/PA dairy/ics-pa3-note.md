...
## Some Tutorial
关于 mcause
![image.png|400](https://kold.oss-cn-shanghai.aliyuncs.com/20251129211558.png)
...
- Then we have to deal with another syscall. its mcause is `0x0`, read the friendly manuel, it's a `SYS_exit`
	-  2025-11-29 阅读 QQ 群刘杰老师 tutorial 发现 `mcause` 并不是 a7 的值，而是在框架代码中总是 `11`。具体应该是一个 M（ecall from m-mode)
	- 这里我们从 `a7` 获得值，但并不是给了 `mcause`
