...
Pseudo-header is not really transmit in the tranport process, but it's included in the checksum, so we make sure we transport the UCP packet to the right IP address
...
|	seq	number					|
|	ack number					|
| Hdrlen | 0 | Flags | Advs Wnd	|
| Checksum		|	Urgent pnter|
| 	Options ( variable)			|
| 			Data				|
```

Sequence numbers are *byte* *offset*
