...
> [!Note] 什么是 CUDA？从 SIMT 简单理解一下
> . SIMT 的本质：逻辑上的“多线程”，物理上的“打包执行”
> 在 SIMT 架构下，你编写的代码看起来像是标量代码（Scalar Code），即你为“一个线程”编写逻辑。但硬件在执行时，会将这些线程打包，让成百上千个线程同时运行。
> 
> 对比 SIMD：在 SIMD（如 Intel 的 AVX）中，你必须显式地操作向量寄存器（比如一次性处理 8 个 float）。
> 
> CUDA 的 SIMT：你只需要写 a[i] + b[i]，GPU 硬件会自动调度 32 个线程组成一个 Warp（线程束）。这 32 个线程在同一时刻执行相同的指令，但处理不同的数据。
